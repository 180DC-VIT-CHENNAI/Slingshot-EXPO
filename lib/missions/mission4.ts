...
> = {
  "Expand\nMarket": {
    correct:
      "Expanding into new markets drives sustainable growth and diversification.",
    wrong:
      "This strategy focuses on growth, but the priority here is strengthening internal operations first.",
  },
  "Reduce\nCost": {
    correct:
      "Reducing costs directly improves margins and competitive positioning.",
    wrong:
      "Cost savings help, but the urgent need is to build long-term market presence.",
  },
  "Improve\nProcess": {
    correct:
      "Streamlining processes eliminates waste and accelerates delivery.",
    wrong:
      "Process gains are valuable, but the bigger opportunity lies in market reach.",
  },
  "Digital\nTransform": {
    correct:
      "Digital transformation unlocks new capabilities and future-proofs the business.",
    wrong:
      "Tech upgrades are important, but the critical move is expanding the customer base.",
  },
  "Price\nStrategy": {
    correct:
      "An optimized pricing strategy maximizes revenue and captures market value.",
...
export const mission4Config: MissionConfig = {
  ...
  drawEnvironment(scene: Phaser.Scene, w: number, h: number) {
    ...
    const skyT = h * (isMobile ? 0.1 : 0.08);
    const skyH = h * 0.22;
    ...
    const tableY = h * (isMobile ? 0.6 : 0.55);
    ...
    STRATEGIES.forEach((strat, i) => {
      ...
      const card = {
        ...
        labelText: scene.add
          ...
          .setOrigin(0.5),
        iconText: scene.add
          .text(0, -(isMobile ? 14 : 20), strat.icon, {
            fontSize: iconSize,
          })
          .setOrigin(0.5),
      ...
      card.correctGlow.fillRoundedRect(
        -halfW - 4,
        -halfH - 4,
        cardW + 8,
        cardH + 8,
  ...
  onHit(
    ...
    distance: number;
    completed: boolean;
    popups?: { title: string; body: string; color?: number }[];
  } {
    const hitIdx = hitTestCard(targets.cards, projectileX, projectileY);
    ...
    if (hitIdx >= 0 && targets.cards[hitIdx].isCorrect) {
      ...
      const label = scene.add
        .text(
          scene.cameras.main.width / 2,
          scene.cameras.main.height * 0.35,
          "CORRECT STRATEGY!",
          {
            fontSize: "28px",
            color: "#FFD700",
    ...
    if (hitIdx >= 0) {
      ...
      const popup = {
        ...
        body:
          ...
          correctLabel +
          " — " +
          STRATEGY_EXPLANATIONS[correctCard.strategy.label].correct,
        color: 0xf44336,
      };
