...
export default function ResultScreen({ hit, message, playerName, onPlayAgain }: ResultScreenProps) {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => setShowSecondaryButtons(true), hit ? 1600 : 1200)
      ...
      <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 80 }}>
        ...
        <div className={`relative mx-6 max-w-sm w-full transition-all duration-700 ${visible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-90'}`}>
          <div className="glass-card p-8 text-center">
            ...
            {showMessage && (
              <div className="mb-2">
                <div className="inline-block px-5 py-2.5 rounded-2xl mb-3"
                     style={{
            ...
            {showSecondaryButtons && (
              <div className="mt-4 mb-2">
                <div className="inline-block px-4 py-2 rounded-xl animate-count-bounce"
                     style={{
                       background: 'rgba(124,252,0,0.15)',
                       border: '1px solid rgba(124,252,0,0.3)',
                     }}>
                  <span className="font-display font-black text-3xl"
                        style={{ color: '#7CFC00', textShadow: '0 0 15px rgba(124,252,0,0.5)' }}>
                    +{countUpScore}
                  </span>
            ...
            {showSecondaryButtons && (
              <div className="space-y-3 mt-4 animate-slide-in-up">
                {showPlayAgain ? (
                  <button onClick={onPlayAgain} className="w-full btn-primary text-sm py-3.5 animate-scale-in">
                    PLAY AGAIN
                  </button>
                ) : (
                  <div className="w-full text-sm py-3.5 text-center font-display font-bold text-white/40"
                       style={{
                ...
                <a href="https://180dc.org" target="_blank" rel="noopener noreferrer" className="w-full btn-secondary text-sm py-3.5 block text-center">
                  VISIT 180DC
                </a>
                <a href="/wall" className="w-full btn-secondary text-sm py-3.5 block text-center">
                  SEE WHO HIT IT
                </a>
              </div>
            )}
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 80 }}>
      <div className={`relative mx-6 max-w-sm w-full transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <div className="text-center">
          {showMessage && (
            <div className="mb-6">
              <div className="inline-block px-5 py-3 rounded-2xl"
                ...
                <p className="font-display font-semibold text-base text-white/90 leading-snug"
                   style={{ minHeight: '1.5em' }}>
                  {message.slice(0, displayedChars)}
                  {displayedChars < message.length && (
          ...
          {showSecondaryButtons && (
            <div className="space-y-3 animate-slide-in-up">
              <button onClick={onPlayAgain} className="w-full btn-primary text-sm py-3.5">
                PLAY AGAIN
              </button>
              <a href="https://180dc.org" target="_blank" rel="noopener noreferrer" className="w-full btn-secondary text-sm py-3.5 block text-center">
                VISIT 180DC
